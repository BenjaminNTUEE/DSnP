...
{
   // TODO...
   if(_dofileStack.size() == 5) {
      cerr << "It has reached max recursion depth (1024)\n";
      return false;
   }
   // dof do1 => 開do1檔，new一個inf物件_dofile指到do1
   // do1中有do2 => 把do1的_dofile存到stack，再new一個新inf指到do2
   ifstream* temp = new ifstream(dof.c_str());
   if(*temp)   // open dofile successed
   {
...
CmdParser::parseCmd(string& option)
{
   ...
   // TODO...
   string cmd;
   // myStrGetTok(str, cmd): cmd = the first word of str( _history.back() )
   size_t begin = myStrGetTok(str, cmd);  // begin = the begin of the remain part of str
   CmdExec* e = getCmd(cmd);
   
   if(!e) { cerr << "Illegal command!! (" << cmd << ")\n"; return NULL; }

   if(begin != string::npos) option = str.substr(begin, str.size()); // option = the remain str
...
// 3. All string comparison are "case-insensitive". // 不區分大小寫的
//
CmdExec*
