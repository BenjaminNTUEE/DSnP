/****************************************************************************
  FileName     [ dbJson.cpp ]
  PackageName  [ db ]
  Synopsis     [ Define database Json member functions ]
  Author       [ Chung-Yang (Ric) Huang ]
  Copyright    [ Copyleft(c) 2015-present LaDs(III), GIEE, NTU, Taiwan ]
****************************************************************************/

#include <iomanip>
#include <iostream>
#include <fstream>
#include <cstdlib>
#include <cassert>
#include <climits>
#include <cmath>
#include <string>
#include <algorithm>
#include "dbJson.h"
#include "util.h"

using namespace std;

/*****************************************/
/*          Global Functions             */
/*****************************************/
ostream&
operator << (ostream& os, const DBJsonElem& j)
{
   os << "\"" << j._key << "\" : " << j._value;
   return os;
}

istream& operator >> (istream& is, DBJson& j)
{
   // TODO: to read in data from Json file and store them in a DB 
   // - You can assume the input file is with correct JSON file format
   // - NO NEED to handle error file format
   assert(j._obj.empty());
   string input, key, value;
   getline(is, input);

   while(1)
   {
       is >> input;       //read the key
       if(input=="}") break;                //break when EOF
       else if(input==",") is >> input;   //ignore , between two elements

       key = key.assign(input, 1, input.size()-2);  //ignore " that mark the key
       is >> input;       //ignore : between key and value
       is >> input;       //read the value of the key
       value = input;
       j.add(DBJsonElem(key, atoi(value.c_str())));
   }
   j._created = true;
   return is;
}

ostream& operator << (ostream& os, const DBJson& j)
{
   // TODO
   cout << "{\n";
   if(j.size()!=0) {
      for(size_t i = 0; i < j.size()-1; i++)
         cout << "  " << j[i] << "," << endl;
      cout << "  " << j[j.size()-1] << endl << "}" << endl;
   }
   else cout << "}\n";

   return os;
}

/**********************************************/
/*   Member Functions for class DBJsonElem    */
/**********************************************/
/*****************************************/
/*   Member Functions for class DBJson   */
/*****************************************/
void
DBJson::reset()
{
   // TODO
   _obj.clear();
   _created = false;
}

// return false if key is repeated
bool
DBJson::add(const DBJsonElem& elm)
{
   // TODO
   for(size_t i = 0; i < _obj.size(); i++)
      if(elm._key == _obj[i]._key) return false;
   _obj.push_back(elm);
   return true;
}

// return NAN if DBJson is empty
float
DBJson::ave() const
{
   // TODO
   if(_obj.size()==0) return NAN;
   int s = 0;
   for(size_t i = 0; i < _obj.size(); i++) s += _obj[i]._value;
   return (float)s / (float)_obj.size();
}

// If DBJson is empty, set idx to size() and return INT_MIN
int
DBJson::max(size_t& idx) const
{
   // TODO
   int maxN = INT_MIN;
   idx = 0;
   if(_obj.size()!=0) {    // not empty
      maxN = _obj[0]._value;
      for(size_t i = 0; i < _obj.size(); i++)
         if(_obj[i]._value > _obj[idx]._value) { idx = i; maxN = _obj[idx]._value; }
   }
   return  maxN;
}

// If DBJson is empty, set idx to size() and return INT_MAX
int
DBJson::min(size_t& idx) const
{
   // TODO
   int minN = INT_MAX;
   idx = 0;
   if(_obj.size()!=0) {    // not empty
      minN = _obj[0]._value;
      for(size_t i = 0; i < _obj.size(); i++)
         if(_obj[i]._value < _obj[idx]._value) { idx = i; minN = _obj[idx]._value; }
   }
   return  minN;
}

void
DBJson::sort(const DBSortKey& s)
{
   // Sort the data according to the order of columns in 's'
   ::sort(_obj.begin(), _obj.end(), s);
}

void
DBJson::sort(const DBSortValue& s)
{
   // Sort the data according to the order of columns in 's'
   ::sort(_obj.begin(), _obj.end(), s);
}

// return 0 if empty
int
DBJson::sum() const
{
   // TODO
   int s = 0;
   if(_obj.size()!=0)   // not empty
      for(size_t i = 0; i < _obj.size(); i++) s += _obj[i]._value;
   return s;
}
