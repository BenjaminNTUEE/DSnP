...
DBAppendCmd::exec(const string& option)
{
   ...
   /*if( !(options[0].c_str()[0]==95     // '_'
       || (options[0].c_str()[0]>=65 && options[0].c_str()[0]<=90)   // A-Z
       || (options[0].c_str()[0]>=97 && options[0].c_str()[0]<=122)  // a-z
      ) ) return CmdExec::errorOption(CMD_OPT_ILLEGAL, options[0]);*/

   int v;
   if(!myStr2Int(options[1], v)) return CmdExec::errorOption(CMD_OPT_ILLEGAL, options[1]);
   /*for(size_t i = 0; i < options[1].size(); i++)
      if( !(options[1].c_str()[i]>=48 && options[1].c_str()[i]<=57) )
         return CmdExec::errorOption(CMD_OPT_ILLEGAL, options[1]);*/
   
   if(!dbjson.add(DBJsonElem(options[0], v))) {
      cerr << "Element with key \"" << options[0] << "\" already exists!!\n";
      return CMD_EXEC_ERROR;
   }
...
{  
   ...
   if (!CmdExec::lexOptions(option, options))
      return CMD_EXEC_ERROR;
   if(options.size()==0) cout << dbjson;
   else if(options.size()==1) {
      string _print = options[0];
      size_t i;
      for(i = 0; i < dbjson.size(); i++)
         if(dbjson[i].key() == _print) break;
      
      if(i < dbjson.size() && _print==dbjson[i].key()) cout << dbjson[i];
      else {
         cerr << "Error: No JSON element with key \"" << _print << "\" is found.\n";
         return CMD_EXEC_ERROR;
      }
   }
